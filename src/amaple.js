		...
		if (!isEmpty(allScriptNames)) {
			...
			if (!isEmpty(deps)) {
				parses.script = parses.script.replace(raddComponents, function (match) {
					return match + ("depComponents:[" + deps.join(",") + "],");
				});
			}
		}
    ...
    initTemplate: function initTemplate(template, scopedStyle) {
        ...
        foreach(scopedStyle, function (styles, selector) {
            ...
            foreach(styles, function (val, styleName) {
                num = parseInt(val);
                styleString += styleName + ":" + (val + (type$1(num) === "number" && (num >= 0 || num <= 0) && noUnitHook.indexOf(styleName) === -1 ? "px" : "")) + ";";
            });
...
/**
	...
	http://amaple.org/######
*/
function Module(moduleElem, vmData) {
	vmData = vmData || {
		init: function init() {
			return {};
		}
	...
	actionLoad: function actionLoad(currentStructure, moduleNode, param, args, data, method, timeout) {
		...
		if ((!method || method.toUpperCase() !== "POST") && historyModule) {
			...
			currentStructure.updateFn = function () {
				...
				historyModule.updateFn(am, { moduleNode: moduleNode, moduleFragment: historyModule.updateFn.moduleFragment.clone(), NodeTransaction: NodeTransaction, require: require, signCurrentRender: signCurrentRender, flushChildren: flushChildren(this) });
			};
			...
			http.request({
				...
				currentStructure.updateFn = function () {
					...
					updateFn(am, { moduleNode: moduleNode, moduleFragment: updateFn.moduleFragment.clone(), NodeTransaction: NodeTransaction, require: require, signCurrentRender: signCurrentRender, flushChildren: flushChildren(this) });
